...
/* ----------------------------------
 *  Connection type (executor name / direct)
 * ---------------------------------- */
export const ConnectionTypeCell = ({
  ...
  if (connection_status !== 'connected') {
    return <Text style={{ justifySelf: 'center' }}>Not available</Text>;
  }

  // Direct‑connected systems have no executor_id
  if (!executor_id) {
    return <Text style={{ justifySelf: 'center' }}>Direct connection</Text>;
  }

  if (!executor_name) {
    return <Text style={{ justifySelf: 'center' }}>—</Text>;
  }
...
/* ----------------------------------
 *  Connection status (with helper renderer)
 * ---------------------------------- */
