...
const SystemsContent = ({ remediationDetails, remediationStatus, refetch }) => {
  ...
  const { result: hostTags } = useRemediationsQuery(getHostTags(axios), {
    params: { host_id_list: systemIds },
  });
  const { result: hostOs } = useRemediationsQuery(getHostOS(axios), {
    params: { host_id_list: systemIds },
  });
  ...
  const mergedSystems = useMemo(() => {
    const counts = {};
    allSystems.forEach(
      (system) => (counts[system.id] = (counts[system.id] || 0) + 1)
    );

    const statusById = (remediationStatus?.connectedData || []).reduce(
      (acc, group) => {
        group.system_ids.forEach((id) => {
          acc[id] = group.connection_status;
        });
        return acc;
      },
      {}
    ...
    allSystems.forEach((system) => {
      if (!map[system.id]) {
        ...
        map[system.id] = {
          ...
          tags: tagsForThis,
          connection_status: statusById[system.id] ?? 'Not configured',
        };
      }
    });

    return Object.values(map);
  }, [allSystems, hostTags, hostOs]);
  ...
  return (
    ...
    >
      ...
      <RemediationsTable
        ...
        ouiaId="ActionsTable"
        items={mergedSystems}
        total={mergedSystems.length}
        columns={tableColumns}
        options={{
          //Known bug in asyncTableTools - needed for bulkSelect
          onSelect: () => '',
          itemIdsInTable: mergedSystems.map((i) => i.id),
          itemIdsOnPage: mergedSystems.map((i) => i.id),
          total: mergedSystems.length,
          actionResolver: () => {
            return [
              {
